...
const emailTest = {
  ...
  verifyEmail(str) {
    ...
    for (let i = 0; i < lp.length; i++) {
      ...
      if (lp[x] === lp[y]) {
        msg = `consecutive dots are not allowed, unless the username is quoted`;
        return { error: true, message: msg };
      }
    }
    ...
    if ((lp.indexOf('@') > -1) && !quoted) {
      msg = `only one @ is allowed outside quotation marks`;
      return { error: true, message: msg };
    }
    ...
    for (let i = 0; i < dom.length; i++) {
      ...
      if (domArr[m] === domArr[n]) {
        msg = `consecutive dots after @ is not allowed.`;
        return { error: true, message: msg };
      }
    }
    ...
    console.log('line 177 emailverifier.js',this);
    ...
    if (dom[0] === '-') {
      msg = `leading dash in front of domain is not allowed`;
      return { error: true, message: msg }
    }
    ...
    for (let i = 0; i < dom.length; i++) {
      if (!this.isValidDomChar(dom[i], dom)) {
        msg = `special characters like " ${dom[i]} " is not allowed in the domain`;
        return { error: true, message: msg };
      }
    }
